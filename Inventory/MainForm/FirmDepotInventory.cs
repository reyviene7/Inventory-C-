...
{
    public partial class FirmDepotInventory : Form
    {
        ...
        public bool ExiInven
        {
            ...
            set
            {
                ...
                if(_extInvent)
                Close();
               var main = new FrmMain(_userId, _usrTyp);
                main.Show();
            }
        }
        public int BranchId
        ...
        public FirmDepotInventory(int userId, int usrTyp)
        {
            _userId = userId;
            _usrTyp = usrTyp;
            InitializeComponent();
        ...
        {
            cmbDIS.Text = Constant.DefaultDepot;
        }
        ...
        private IEnumerable<ViewDepot> RebindInventory()
        {
            using (var session = new DalSession())
            {
                var unWork = session.UnitofWrk;
        ...
        private ViewDepot ShowEntity(int inventoryId)
        {
            using (var session = new DalSession())
            {
                var unWork = session.UnitofWrk;
        ...
        private void txtDEL_Leave(object sender, EventArgs e)
        {
            ...
            if (len > 0)
            {
                ...
                if (verify > 0)
                {
                    txtDEL.Focus();
                    txtDEL.BackColor = Color.Yellow;
                    PopupNotification.PopUpMessages(0, "Delivery No: " + deliveryNo + " already exist!", Messages.GasulPos);
                }
            }
        }
